...
export function LendBorrowPage() {
  ...
  function handleDelete(id: string) {
    if (window.confirm("Delete this lend / borrow entry?")) deleteLendBorrowEntry(id);
  }

  function markClosed(item: LendBorrowEntry) {
    void updateLendBorrowEntry(item.id, { ...item, amountSettled: item.amount });
  }
  ...
  return (
    <AppShell>
            ...
            <Card key={`${item.id}-mobile`} className={`p-4 md:hidden ${item.status.overdue ? "border-danger/30" : ""}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2"><p className="truncate font-semibold">{item.person}</p><Badge tone={item.type === "given" ? "green" : "red"}>{item.type === "given" ? "Given" : "Borrowed"}</Badge></div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-2xl font-semibold tabular-nums text-primary">{formatCurrency(item.balance, state.userSettings.currency)}</p>
                </div>
                <div className="min-w-0 text-right">
                  <p className="text-muted">Settled</p>
                  <p className="mt-1 text-2xl font-semibold tabular-nums">{formatCurrency(item.amountSettled, state.userSettings.currency)}</p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-4 grid gap-3 text-sm text-muted sm:grid-cols-2 lg:grid-cols-4">
                    <div><p>Total amount</p><p className="mt-1 font-semibold tabular-nums text-white">{formatCurrency(item.amount, state.userSettings.currency)}</p></div>
                    <div><p>Settled</p><p className="mt-1 font-semibold tabular-nums text-white">{formatCurrency(item.amountSettled, state.userSettings.currency)}</p></div>
                    <div><p>Balance</p><p className={`mt-1 font-semibold tabular-nums ${item.balance > 0 ? "text-primary" : "text-success"}`}>{formatCurrency(item.balance, state.userSettings.currency)}</p></div>
      ...
      <Modal open={Boolean(editingItem)} onClose={() => setEditingItem(null)} title="Edit lend / borrow entry" description="Update person, balance, or due date.">
